...
assert.deepEqual(tics, { birdies: 1, eagles: 0, skins: 3, front: 1, frontWeight: 2, back: 1, backWeight: 2, totalNet: 1, totalNetWeight: 2, sandyPars: 1, sandyBirdies: 0, kps: 1, kpFails: 0, total: 9, weightedTics: 12, pointsEarned: 6 });
...
assert.equal(E.ticSummary(alice, [alice, bob], E.COURSE, supplantedKpSettings).kpFails, 1);
assert.equal(E.ticSummary(bob, [alice, bob], E.COURSE, supplantedKpSettings).kps, 1);
assert.equal(E.ticSummary(bob, [alice, bob], E.COURSE, supplantedKpSettings).kpFails, 0);
const failedCurrent = { ...bob, scores: [...bob.scores] };
failedCurrent.scores[1] = 4;
assert.equal(E.kpClaimStatus(failedCurrent, E.COURSE, supplantedKpSettings, 1), "failed");
assert.equal(E.ticSummary(failedCurrent, [alice, failedCurrent], E.COURSE, supplantedKpSettings).kps, 0);
assert.equal(E.ticSummary(failedCurrent, [alice, failedCurrent], E.COURSE, supplantedKpSettings).kpFails, 1);
...
assert.equal(E.ticSummary(pendingCurrent, [alice, pendingCurrent], E.COURSE, supplantedKpSettings).kpFails, 0);
